...
describe("index.js", () => {
  ...
  it('Sets auth headers correctly', () => {
    ...
    assert.equal(client.config.headers["client-id"], "my-client-id");
    assert.equal(client.config.headers["authorization"], "Bearer my-app-access-token");
  });
  ...
  it('Should deeply merge options', () => {
    ...
    assert.equal(Object.keys(client.config.headers).length, normalHeaders + 1);
  });
